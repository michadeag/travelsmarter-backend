...
exports.generateTippingPdf = async (req, res) => {
  try {
    const { email, firstName, country, billAmount } = req.body;
    if (!email || !country) {
    ...
    await pool.query(
      ...
      ['tipping-calculator', firstName || null, 'tipping-calculator',
        JSON.stringify({ country, billAmount }), JSON.stringify(result)]
    );
