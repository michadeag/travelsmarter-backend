...
exports.generateVisaPdf = async (req, res) => {
  try {
    const { email, firstName, sourcePage, country, travelDate, passportExpiryDate } = req.body;
    if (!email || !country) {
    ...
    const leadResult = await pool.query(
      ...
      [email, firstName || null, 'visa-requirement-checker',
        JSON.stringify({ country, travelDate, passportExpiryDate }), JSON.stringify(result), sourcePage || null]
    );
