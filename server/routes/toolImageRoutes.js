...
// Kick off (or re-kick-off with force=true) Ideogram generation for every
// free-tool category's branded thumbnail. Returns immediately — generation
// continues in the background; poll GET / to watch results fill in.
...
router.get('/', async (req, res) => {
  try {
    const images = await toolOgImageService.getAllToolImages();
    res.json({ success: true, total: toolOgImageService.TOOL_THEMES.length, images });
  } catch (err) {
    res.status(500).json({ success: false, message: err.message });
  }
});
